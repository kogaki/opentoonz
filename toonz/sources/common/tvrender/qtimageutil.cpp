...
std::vector<TStroke*> convertQPainterPathToStrokes(const QPainterPath pathes){
	...
	for(int i=0; i<pathes.elementCount();){
		...
		if(element.isMoveTo()){ // stroke start
			...
			auto lastPoint = TThickPoint(lastElement.x, lastElement.y, 0);
			points.push_back(lastPoint);
			points.push_back((point-lastPoint)*1.5 + lastPoint); // I really don't know why x1.5 goes well
		}else if(element.isLineTo()){
			auto lastElement = pathes.elementAt(i-1);
			auto lastPoint = TThickPoint(lastElement.x, lastElement.y, 0);
			points.push_back(lastPoint);
			points.push_back(lastPoint + (point - lastPoint)*0.25);
			points.push_back(lastPoint + (point - lastPoint)*0.5);
			points.push_back(lastPoint + (point - lastPoint)*0.75);
		}
		i++;
	}
	if(!points.empty()){
